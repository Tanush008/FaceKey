...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:Tanu*12345@localhost:5433/fackeyUsers'
...
def get_blink_ratio(eye_points, facial_landmarks):
    corner_left = (facial_landmarks.part(
        eye_points[0]).x, facial_landmarks.part(eye_points[0]).y)
    corner_right = (facial_landmarks.part(
    ...
    while True:
        ...
        if process_this_frame:
            ...
            for face in faces:
                landmarks = predictor(frame, face)
                left_eye_ratio = get_blink_ratio(left_eye_landmarks, landmarks)
                right_eye_ratio = get_blink_ratio(
                    right_eye_landmarks, landmarks)
                blink_ratio = (left_eye_ratio + right_eye_ratio) / 2
...
def upload():
    if request.method == 'POST':
        ...
        try:
            user = User(name=name, regNum=regNum, password=password, image_path=os.path.join(
